    ...
    public function showForUser(Request $request)
    {
        $travelPackage = TravelPackage::query()
            ->when($request->price, function($query) use ($request) {
                $direction = $request->price === 'high_to_low' ? 'desc' : 'asc';
                $query->orderBy('price_start_from', $direction);
            })
            ->when($request->categories, function($query) use ($request) {
                $query->whereIn('tour_type', $request->categories);
            })
            ->paginate(9);

        return view('user.package', compact('travelPackage'));
    }
